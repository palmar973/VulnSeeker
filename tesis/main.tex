...
\begin{itemize}
    ...
    dispone de señal observable. Su detección exigiría inferencia por diferencia
    de comportamiento o canales fuera de banda.
    ...
    el cuerpo observable, escenario que requeriría detección basada en tiempo.
...
identificadas (XSS sobre \texttt{POST}, detección ciega por tiempo y ampliación de
vectores) que se recogen en las recomendaciones.
...
por los resultados de la suite de pruebas unitarias: los 180~tests
...
\begin{enumerate}
    ...
    límites concretos la detección ciega de \textit{path traversal} (cuando la
    respuesta no refleja el contenido del archivo) y los vectores de inyección por
    cabecera o cuerpo XML no contemplados por el motor; ambos abordables en trabajo
    futuro sin alterar la arquitectura modular. Una tercera carencia detectada en
...
\begin{enumerate}
    ...
    Índice de Youden: incorporar detección de inyección ciega basada en tiempo
    (\textit{path traversal} y \textit{command injection} sin reflejo en la
    respuesta), soportar los vectores de cabecera y cuerpo XML, y añadir
    \textit{payloads} de XSS sensibles al contexto (atributos HTML y bloques
    JavaScript), lo que permitiría una comparación aún más directa con los motores
    comerciales sobre el mismo \textit{ground truth}.
