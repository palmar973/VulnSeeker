...
se realizó mediante 145~pruebas unitarias automatizadas y escaneos controlados
...
diagnóstica. El primer nivel consistió en la construcción de una suite de 145~pruebas
...
\begin{table}[H]
    ...
    \begin{tabular}{p{4.5cm} c c c}
        ...
        Fingerprinter (core) & 6 & 6  & \checkmark \\
        \midrule
        \textbf{TOTAL}      & \textbf{145} & \textbf{145} & \textbf{100\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
identificó 496~puntos de entrada únicos (endpoints y formularios), y cada
módulo evaluó todos los puntos de entrada descubiertos.
...
\begin{table}[H]
    ...
    \begin{tabular}{l r}
        ...
        Puntos de entrada descubiertos & 496 \\
...
reportes independientes por vulnerabilidad. Por ejemplo, los 496~puntos de
entrada descubiertos carecen del header \texttt{X-Frame-Options}, generando
...
detectadas de forma uniforme en los 496~puntos de entrada descubiertos:
...
\begin{table}[H]
    \centering
    \caption{Precisión observada por módulo contra DVWA (Scan~\#90)}
    \label{tab:efectividad}
    \small
    \begin{tabular}{p{3.8cm} r c c}
        \toprule
        \textbf{Módulo} & \textbf{Hallazgos (VP)} & \textbf{FP observados} &
        \textbf{Precisión observada} \\
        \midrule
        SQL Injection       & 331       & 0 & 100\% \\
        XSS (Reflected)     & 245       & 0 & 100\% \\
        Auth / Session      & 173       & 0 & 100\% \\
        Command Injection   & detectado & 0 & 100\% \\
        LFI                 & detectado & 0 & 100\% \\
        Header Analyzer     & 2.480     & 0 & 100\% \\
        CSRF Auditor        & detectado & 0 & 100\% \\
        TLS / HTTPS         & 496       & 0 & 100\% \\
        HSTS                & 496       & 0 & 100\% \\
        Server Disclosure   & 496       & 0 & 100\% \\
        Path Fuzzer         & detectado & 0 & 100\% \\
        \bottomrule
    \end{tabular}
\end{table}

Los módulos de inyección (SQLi, XSS, Command Injection, LFI) detectaron las
vulnerabilidades correspondientes documentadas en DVWA sin generar falsos
positivos observados durante el escenario experimental definido, lo cual se
atribuye al uso de payloads específicos y al análisis de patrones de error en
las respuestas HTTP. Los módulos de configuración (Headers, TLS, HSTS, Server
Disclosure) reportaron 496~hallazgos cada uno, correspondientes a los
496~puntos de entrada descubiertos, lo cual constituye una detección
verdaderamente positiva dado que DVWA opera sobre HTTP sin ningún header de
...
por los resultados de la suite de pruebas unitarias: los 145~tests
...
\begin{enumerate}
    ...
    \item \textbf{Objetivo 4 — Validación experimental:} Las 145~pruebas
...
\begin{enumerate}
    ...
    \item \textbf{Entorno de validación controlado:} La medición de precisión
    (100\%, 0~falsos positivos observados) se obtuvo contra DVWA, un entorno
    intencionalmente vulnerable, y se complementó con un escaneo exploratorio
    sobre la plataforma pública de pruebas Altoro Mutual. Estos resultados
    reflejan contextos de prueba específicos y podrían variar en aplicaciones
    de producción con configuraciones de seguridad parciales o comportamientos
    ambiguos. La validación contra un conjunto más amplio de objetivos con
    \textit{ground truth} etiquetado (p.~ej. OWASP Benchmark, Juice Shop) queda
    planteada como trabajo futuro para cuantificar el recall y el Índice de
    Youden del framework.
\end{enumerate}
