...
% Español académico (APA): usar "Tabla" en vez del "Cuadro" por defecto de babel,
% para que los rótulos coincidan con las referencias del texto ("Tabla X").
...
{\itshape
A mis padres, Claudio Enrique Palmar Alvarado y Mayde Marina León González.

Por ser mi refugio constante y la fuerza detrás de cada uno de mis pasos. A mi papá, por dedicar su vida a la mía, por brindarme un techo firme y por el amor, el cuidado y los consejos de todos los días. A mi mamá, por su apoyo incondicional que nunca conoció de distancias, por su generosidad absoluta y por poner en mis manos las herramientas con las que construí mi futuro.

Por darlo todo por mí, por ser los mejores padres que Dios me pudo regalar y porque sin su sacrificio infinito, este logro habría sido imposible. Esta tesis lleva mi nombre, pero es completamente de ustedes.
\par}
...
A Dios, por ser el principio y el fin de todo. Por el regalo de la vida, la salud y por rodearme de mi familia, mis amigos y Yola. Gracias por moldear a la persona que soy hoy, por sembrar en mí esta inmensa pasión por la computación que hoy rinde sus frutos, por cuidarme siempre, llenarme de paz espiritual y por ser mi amparo y salvador eterno en cada paso de este camino.

A mis padres, Claudio Enrique y Mayde Marina, por ser los pilares fundamentales de mi existencia. Papá, eres la persona más fuerte, trabajadora y honrada que conozco; gracias por ser mi cobijo, por hacer a un lado tu propia vida para dedicar tu esfuerzo a la mía, por cuidarme cada día, por cocinarme, por tus consejos y por tu compañía constante. Mamá, gracias por nunca dejarme de lado, por sostenernos con tu esfuerzo y por darme las herramientas literales para construir este sueño: por esa computadora sin la cual terminar esta carrera habría sido imposible, por tu apoyo incondicional y tu cuidado infinito. A ambos les debo mi mayor gratitud por sacrificar sus propias oportunidades para construir las mías. Gracias por ser ese cimiento sólido al que siempre pude sostenerme, por tolerar mis momentos complejos con una paciencia a prueba de todo, y por estar al pie del cañón en cada victoria y cada etapa de este camino universitario sin vacilar jamás. Sin ustedes dos, nada de esto existiría.

A mis hermanos, Luis Enrique y Génesis Carolina. Luis, porque a pesar de la distancia y las circunstancias, eres mi hermano, te quiero y agradezco genuinamente el apoyo que nos envías, que ha sido parte vital de mi sustento. Génesis, gracias por tu cariño sincero y por haber estado siempre dispuesta a ayudarme y a compartir conmigo en cada oportunidad que hemos tenido.

A mi novia, Yolanda. Mi mejor amiga, mi compañera y mi lugar seguro. Gracias por todo tu amor, por tu apoyo incondicional, por ser mi auxilio en los momentos de estrés y por cada consejo que me ayudó a seguir adelante. Gracias por tu maravillosa compañía y por amarme de una forma tan excepcional.

A mi tutor, el Profesor Sigerist Rodríguez, por su apoyo constante a lo largo de toda la carrera, durante mis pasantías, el servicio comunitario y en el desarrollo de esta tesis. Agradezco que respetara mi idea original, que me diera la libertad de desarrollar mi propia visión y cada consejo preciso. Extiendo este agradecimiento a todos y cada uno de mis profesores; gracias porque todos me enseñaron, todos me moldearon, y de cada uno me llevo lecciones invaluables que formaron al profesional que soy hoy.

A mis amigos de la universidad, gracias por las risas y por hacer de esta etapa algo más divertido, más pasajero y, sencillamente, mejor.

A la Universidad del Zulia, mi \textit{alma máter}, por ser la casa donde forjé mi vocación, y a la institución Caproluz, por haberme abierto sus puertas y brindarme la oportunidad de dar mis primeros pasos profesionales.

A todos los que, de una forma u otra, formaron parte de este proceso: gracias.
...
se realizó mediante 180~pruebas unitarias automatizadas, escaneos controlados
sobre entornos de práctica estándar y una medición cuantitativa contra el OWASP
Benchmark. Los resultados demostraron que la integración
...
Benchmark para obtener el Índice de Youden cuantitativo del framework se desarrolla
en el Capítulo~IV (Resultados) y constituye la validación experimental de mayor
rigor metodológico del presente trabajo, al apoyarse en un \textit{ground truth}
exhaustivamente etiquetado.
...
condicionales anidados ni conocimiento de la implementación subyacente. La
Figura~\ref{fig:arquitectura} ilustra esta organización: el motor depende
exclusivamente de la interfaz abstracta \texttt{ScannerModule}, de la que heredan
los 25~módulos de detección sin acoplamiento lateral entre sí.

\begin{figure}[H]
\centering
\singlespacing
\begin{tikzpicture}[
    cls/.style={rectangle, draw=black, thick, align=center, font=\small,
                inner sep=5pt, minimum height=8mm},
    nota/.style={font=\footnotesize}
]
    \node[cls, fill=orange!15, text width=5.4cm] (engine)
        {\textbf{Engine}\\\footnotesize Motor de orquestación multihilo};
    \node[cls, fill=blue!12, text width=6.4cm, below=14mm of engine] (iface)
        {\textit{$\ll$\,interfaz\,$\gg$}\\\textbf{ScannerModule}\\
         \footnotesize + run(target) : List[Vulnerability]};
    \node[cls, fill=gray!12, text width=2.7cm, below=16mm of iface] (m2)
        {XSSScanner};
    \node[cls, fill=gray!12, text width=2.7cm, left=8mm of m2] (m1)
        {SQLInjection\\Scanner};
    \node[cls, fill=gray!12, text width=2.7cm, right=8mm of m2] (m3)
        {CVELookup\\Scanner};
    \node[nota, right=6mm of m3] (dots) {\dots\ (25)};
    \draw[-{Latex[length=2.6mm]}, thick, dashed] (engine) --
        node[right, nota] {delega} (iface);
    \draw[-{Triangle[open, length=3mm, width=3.4mm]}, thick] (m1.north) -- (iface.south);
    \draw[-{Triangle[open, length=3mm, width=3.4mm]}, thick] (m2.north) -- (iface.south);
    \draw[-{Triangle[open, length=3mm, width=3.4mm]}, thick] (m3.north) -- (iface.south);
\end{tikzpicture}
\caption[Arquitectura modular de VulnSeeker bajo el patrón Strategy]{Arquitectura
modular de VulnSeeker bajo el patrón Strategy. El motor delega en la interfaz
\texttt{ScannerModule}; cada módulo concreto es una estrategia intercambiable que
la implementa.}
...
\subsection{Análisis y Diseño del Sistema}

Con anterioridad a la construcción, VulnSeeker fue objeto de un análisis y diseño
formal que estableció su modelo de proceso de desarrollo, la frontera de sus
entradas y salidas, el flujo de la información entre sus componentes y la
estructura de su almacenamiento persistente. Esta sección documenta dichos
artefactos, que constituyen los planos de ingeniería sobre los cuales se erigió
la herramienta.

\subsubsection{Ciclo de Vida del Desarrollo}

El desarrollo de VulnSeeker siguió un \textbf{modelo de ciclo de vida iterativo e
incremental} (\cite{sommerville2011}), en el cual el sistema se construyó mediante
incrementos funcionales sucesivos en lugar de una única entrega monolítica. Cada
incremento ---registrado en el repositorio de control de versiones como una
\textit{fase}--- incorporó un módulo de detección completo y verificado sobre una
base estable: se especificaba su comportamiento, se implementaba heredando de la
interfaz \texttt{ScannerModule}, se cubría con pruebas unitarias y se integraba al
motor de orquestación antes de iniciar el siguiente incremento.

Este modelo resultó el idóneo para el proyecto por tres razones. En primer lugar,
la arquitectura modular permite que cada incremento sea independiente y
verificable de forma aislada, sin efectos colaterales sobre los módulos ya
estabilizados. En segundo lugar, la retroalimentación obtenida de cada validación
---por ejemplo, la depuración de falsos positivos descrita en el Capítulo~IV---
se incorporaba directamente al diseño del siguiente incremento. En tercer lugar,
el enfoque reduce el riesgo del proyecto, pues un fallo en un módulo nuevo nunca
compromete la funcionalidad ya entregada. Las \textit{Fases Metodológicas} que se
describen a continuación constituyen las iteraciones de mayor nivel de este ciclo.

\subsubsection{Tecnologías y Herramientas de Desarrollo}

La Tabla~\ref{tab:tecnologias} sistematiza el conjunto de tecnologías y
herramientas que conforman el entorno de desarrollo y ejecución de VulnSeeker,
cuya justificación detallada se expuso en la Memoria de Materiales.

\begin{table}[H]
    \centering
    \caption{Tecnologías y herramientas de desarrollo de VulnSeeker}
    \label{tab:tecnologias}
    \small
    \begin{tabular}{p{3.6cm} p{4.4cm} p{5.2cm}}
        \toprule
        \textbf{Categoría} & \textbf{Tecnología / herramienta} & \textbf{Función en el sistema} \\
        \midrule
        Lenguaje base & Python 3.10+ (entorno 3.14) & Núcleo, módulos de detección y lógica de orquestación \\
        Cliente HTTP y \textit{parsing} & \texttt{requests}, \texttt{BeautifulSoup} & Peticiones y análisis de respuestas HTML \\
        Interfaz gráfica & CustomTkinter & GUI de escritorio \\
        Persistencia & SQLite & Historial de escaneos y hallazgos \\
        IA generativa & Llama 3.3 70B vía Groq & Informe ejecutivo y priorización \\
        Inteligencia de CVE & NVD API 2.0 (NIST) & Consulta de vulnerabilidades conocidas \\
        Generación de reportes & ReportLab & Exportación a PDF \\
        Pruebas automatizadas & \texttt{pytest} & Suite de 180 pruebas unitarias \\
        Integración continua & GitHub Actions & Ejecución de la suite en cada \textit{commit} \\
        Control de versiones & Git / GitHub & Gestión del código y de los incrementos \\
        \bottomrule
    \end{tabular}
\end{table}

\subsubsection{Entradas, Procesos y Salidas}

En su nivel más abstracto, el sistema puede describirse mediante el modelo clásico
de entradas, procesos y salidas (Tabla~\ref{tab:eps}): recibe un objetivo y su
configuración, ejecuta una cadena de procesos de reconocimiento, detección y
análisis, y produce un conjunto de reportes y registros persistentes.

\begin{table}[H]
    \centering
    \caption{Entradas, procesos y salidas del sistema}
    \label{tab:eps}
    \small
    \begin{tabular}{p{4.6cm} p{4.6cm} p{4.6cm}}
        \toprule
        \textbf{Entradas} & \textbf{Procesos} & \textbf{Salidas} \\
        \midrule
        \textbullet~URL objetivo \newline
        \textbullet~Configuración (User-Agent, cookies, profundidad de \textit{crawl}) \newline
        \textbullet~Credenciales de sesión \newline
        \textbullet~Clave de API de Groq
        &
        \textbullet~Reconocimiento (OSINT, \textit{fingerprinting}, \textit{crawling}) \newline
        \textbullet~Descubrimiento API-aware \newline
        \textbullet~Ataque multihilo (25 módulos) \newline
        \textbullet~Deduplicación y clasificación por severidad \newline
        \textbullet~Análisis con IA
        &
        \textbullet~Reporte ejecutivo en PDF \newline
        \textbullet~Exportación en JSON y CSV \newline
        \textbullet~Hallazgos clasificados por severidad \newline
        \textbullet~Registro histórico en SQLite
        \\
        \bottomrule
    \end{tabular}
\end{table}
...
El diagrama de flujo de datos (DFD) describe el sistema desde la perspectiva del
movimiento de la información, con independencia de su implementación. El nivel~0
o diagrama de contexto (Figura~\ref{fig:dfd0}) representa a VulnSeeker como un
único proceso que intercambia datos con cuatro entidades externas: el usuario, el
sitio web auditado y los servicios de la NVD del NIST y de Groq.

\begin{figure}[H]
\centering
\singlespacing
\begin{tikzpicture}[
    ext/.style={rectangle, draw=black, thick, fill=gray!12, align=center,
                font=\small, text width=2.7cm, minimum height=9mm},
    proc/.style={circle, draw=black, thick, fill=blue!12, align=center,
                 font=\small, minimum size=2.1cm},
    flujo/.style={{Latex[length=2.2mm]}-{Latex[length=2.2mm]}, thick},
    etq/.style={font=\footnotesize}
]
    \node[proc] (vs) {\textbf{0}\\ VulnSeeker};
    \node[ext, left=3cm of vs] (user) {Usuario / Auditor};
    \node[ext, right=3cm of vs] (nvd) {NVD API (NIST)};
    \node[ext, above=9mm of nvd] (site) {Sitio web objetivo};
    \node[ext, below=9mm of nvd] (groq) {Groq (Llama 3.3)};
    \draw[flujo] (user) -- node[etq, above]{URL, config / reportes} (vs);
    \draw[flujo] (vs) -- node[etq, sloped, above]{HTTP / respuestas} (site);
    \draw[flujo] (vs) -- node[etq, above]{tech / CVEs} (nvd);
    \draw[flujo] (vs) -- node[etq, sloped, below]{hallazgos / informe} (groq);
\end{tikzpicture}
\caption{Diagrama de flujo de datos de nivel 0 (diagrama de contexto)}
\label{fig:dfd0}
\end{figure}

El nivel~1 (Figura~\ref{fig:dfd1}) descompone el proceso principal en sus cuatro
subprocesos ---reconocimiento, detección, análisis e informe--- y revela el
almacén de datos \texttt{vulns.db} donde se persisten los escaneos y los
hallazgos.
...
\begin{tikzpicture}[
    ext/.style={rectangle, draw=black, thick, fill=gray!12, align=center,
                font=\footnotesize, text width=2.2cm, minimum height=8mm},
    proc/.style={rectangle, rounded corners, draw=black, thick, fill=blue!10,
                 align=center, font=\small, text width=3.6cm, minimum height=9mm},
    alm/.style={rectangle, draw=black, thick, fill=green!12, align=center,
                font=\footnotesize, text width=3.6cm, minimum height=7mm},
    f/.style={-{Latex[length=2.2mm]}, thick}
]
    \node[proc] (p1) {\textbf{1.}~Reconocimiento};
    \node[proc, below=9mm of p1] (p2) {\textbf{2.}~Detección (25 módulos)};
    \node[proc, below=9mm of p2] (p3) {\textbf{3.}~Análisis con IA};
    \node[proc, below=9mm of p3] (p4) {\textbf{4.}~Informe y persistencia};
    \node[alm, below=9mm of p4] (db) {Almacén: \texttt{vulns.db}};
    \node[ext, left=2.5cm of p1] (user) {Usuario};
    \node[ext, right=2.5cm of p1] (site) {Sitio objetivo};
    \node[ext, right=2.5cm of p2] (nvd) {NVD};
    \node[ext, right=2.5cm of p3] (groq) {Groq};
    \draw[f] (user) -- node[font=\footnotesize, above]{URL} (p1);
    \draw[f] (p1) -- node[right, font=\footnotesize]{superficie} (p2);
    \draw[f] (p2) -- node[right, font=\footnotesize]{hallazgos} (p3);
    \draw[f] (p3) -- node[right, font=\footnotesize]{informe} (p4);
    \draw[f] (p4) -- node[right, font=\footnotesize]{escaneos} (db);
    \draw[{Latex[length=2.2mm]}-{Latex[length=2.2mm]}, thick] (p1) -- (site);
    \draw[{Latex[length=2.2mm]}-{Latex[length=2.2mm]}, thick] (p2) -- (nvd);
    \draw[{Latex[length=2.2mm]}-{Latex[length=2.2mm]}, thick] (p3) -- (groq);
    \draw[f] (p4.west) to[bend left=18] node[left, font=\footnotesize]{reportes} (user.south);
\end{tikzpicture}
\caption{Diagrama de flujo de datos de nivel 1}
\label{fig:dfd1}
\end{figure}

\subsubsection{Modelo de Datos: Entidad-Relación y Diccionario}

La persistencia de VulnSeeker se implementa sobre una base de datos SQLite
estructurada en cuatro entidades. La Figura~\ref{fig:er} presenta el modelo
entidad-relación: cada escaneo (\texttt{scans}) agrupa las vulnerabilidades y los
subdominios que se le asocian mediante una clave foránea, en sendas relaciones de
uno a muchos; la entidad \texttt{settings} almacena la configuración del sistema
...
\singlespacing
\begin{tikzpicture}[
    ent/.style={rectangle, draw=black, thick, align=left, font=\footnotesize, inner sep=4pt},
    rel/.style={font=\footnotesize, fill=white, inner sep=1pt}
]
    \node[ent, fill=orange!12] (scans)
        {\textbf{scans}\\ \underline{id} : INTEGER\\ target\_url : TEXT\\
         scan\_date : TEXT\\ total\_vulns : INTEGER\\ technologies : TEXT};
    \node[ent, fill=gray!12, right=20mm of scans] (settings)
        {\textbf{settings}\\ \underline{key} : TEXT\\ value : TEXT};
    \node[ent, fill=blue!8, below=18mm of scans, xshift=-28mm] (vulns)
        {\textbf{vulnerabilities}\\ \underline{id} : INTEGER\\ \textit{scan\_id} : INTEGER (FK)\\
         name : TEXT\\ description : TEXT\\ severity : INTEGER\\
         target\_url : TEXT\\ payload : TEXT};
    \node[ent, fill=blue!8, below=18mm of scans, xshift=32mm] (subs)
        {\textbf{subdomains}\\ \underline{id} : INTEGER\\ \textit{scan\_id} : INTEGER (FK)\\
         subdomain : TEXT\\ status : TEXT};
    \draw[thick] (scans.south) -- node[rel, near start]{1}
        node[rel, midway]{\textit{contiene}} node[rel, near end]{N} (vulns.north);
    \draw[thick] (scans.south) -- node[rel, near start]{1}
        node[rel, midway]{\textit{descubre}} node[rel, near end]{N} (subs.north);
\end{tikzpicture}
\caption{Modelo entidad-relación de la base de datos de VulnSeeker}
\label{fig:er}
\end{figure}

El diccionario de datos correspondiente (Tabla~\ref{tab:diccionario}) detalla, para
cada entidad, sus campos con el tipo, las restricciones de integridad y la
descripción semántica.

\begin{table}[H]
    \centering
    \caption{Diccionario de datos de la base de datos de VulnSeeker}
    \label{tab:diccionario}
    \small
    \begin{tabular}{p{2.7cm} p{1.9cm} p{2.6cm} p{4.8cm}}
        \toprule
        \textbf{Campo} & \textbf{Tipo} & \textbf{Restricción} & \textbf{Descripción} \\
        \midrule
        \multicolumn{4}{l}{\textit{Entidad} \texttt{scans} \textit{--- escaneos realizados}} \\
        id & INTEGER & PK, autoincr. & Identificador único del escaneo \\
        target\_url & TEXT & NOT NULL & URL objetivo auditada \\
        scan\_date & TEXT & NOT NULL & Fecha y hora (ISO 8601) \\
        total\_vulns & INTEGER & DEFAULT 0 & Número de hallazgos del escaneo \\
        technologies & TEXT & DEFAULT 'Unknown' & Tecnologías detectadas \\
        \midrule
        \multicolumn{4}{l}{\textit{Entidad} \texttt{vulnerabilities} \textit{--- hallazgos}} \\
        id & INTEGER & PK, autoincr. & Identificador del hallazgo \\
        scan\_id & INTEGER & FK $\rightarrow$ scans & Escaneo al que pertenece \\
        name & TEXT & NOT NULL & Nombre de la vulnerabilidad \\
        description & TEXT & --- & Descripción técnica \\
        severity & INTEGER & NOT NULL & Severidad (0 INFO -- 4 CRITICAL) \\
        target\_url & TEXT & --- & URL o \textit{endpoint} afectado \\
        payload & TEXT & --- & Vector de prueba empleado \\
        \midrule
        \multicolumn{4}{l}{\textit{Entidad} \texttt{subdomains} \textit{--- subdominios OSINT}} \\
        id & INTEGER & PK, autoincr. & Identificador del subdominio \\
        scan\_id & INTEGER & FK $\rightarrow$ scans & Escaneo al que pertenece \\
        subdomain & TEXT & NOT NULL & Subdominio descubierto \\
        status & TEXT & DEFAULT 'LIVE' & Estado de actividad \\
        \midrule
        \multicolumn{4}{l}{\textit{Entidad} \texttt{settings} \textit{--- configuración}} \\
        key & TEXT & PK & Clave del parámetro \\
        value & TEXT & NOT NULL & Valor del parámetro \\
        \bottomrule
    \end{tabular}
\end{table}
...
diagnóstica. El primer nivel consistió en la construcción de una suite de 180~pruebas
...
\begin{table}[H]
    ...
    \begin{tabular}{p{4.5cm} c c c}
        ...
        \midrule
        SQL Injection       & 17 & 17 & \checkmark \\
        Cross-Site Scripting & 5  & 5  & \checkmark \\
        Command Injection   & 6  & 6  & \checkmark \\
        Local File Inclusion & 7  & 7  & \checkmark \\
        Injection Points (modules) & 9 & 9 & \checkmark \\
        ...
        Benchmark Scorer (tools) & 7 & 7 & \checkmark \\
        Benchmark Runner (tools) & 6 & 6 & \checkmark \\
        \midrule
        \textbf{TOTAL}      & \textbf{194} & \textbf{194} & \textbf{100\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
condición que DVWA no garantiza por sí sola. Por esta razón, sobre los escaneos
controlados el presente trabajo reporta la precisión observada, y aborda la
medición formal del recall y del Índice de Youden mediante una validación dedicada
contra el OWASP Benchmark Project ---único entorno con \textit{ground truth}
exhaustivamente etiquetado---, cuyos resultados se presentan más adelante en este
mismo capítulo (Tablas~\ref{tab:benchmark-atacable} y~\ref{tab:benchmark-global}).
Esta separación metodológica
evita sobreestimar la cobertura real del framework, en coherencia con las
limitaciones reconocidas más adelante.
...
\subsection{Validación Cuantitativa contra el OWASP Benchmark: Índice de Youden}

A diferencia de DVWA, Altoro Mutual y Juice Shop ---donde el conteo de hallazgos
es cualitativo porque carecen de un inventario exhaustivo y verificado de sus
vulnerabilidades---, el \textit{OWASP Benchmark Project} (v1.2) es una aplicación
Java deliberadamente vulnerable que acompaña cada uno de sus 2.740~casos de prueba
con su etiqueta de \textit{ground truth}: para cada caso se conoce con certeza si
la vulnerabilidad es real o si se trata de un \textit{sink} seguro. Esta condición
habilita el cálculo de las métricas formales de recall ($TPR$), tasa de falsos
positivos ($FPR$) e Índice de Youden ($J = TPR - FPR$) que el Capítulo~III definió
como instrumento de \textit{benchmarking}, y que hasta esta sección sólo era
posible plantear como trabajo futuro.

\subsubsection{Diseño del experimento y honestidad metodológica}

El Benchmark distribuye sus casos en 11~categorías. Siete de ellas
---criptografía débil, \textit{hashing} inseguro, aleatoriedad débil,
\textit{trust boundary}, \textit{cookies} inseguras, inyección LDAP y XPath---
sólo son detectables mediante análisis estático del código fuente (SAST) o quedan
fuera del repertorio de módulos del framework, por lo que evaluarlas contra un
escáner dinámico (DAST) carecería de sentido metodológico. El experimento se
circunscribe, en consecuencia, a las \textbf{cuatro categorías que VulnSeeker
ataca dinámicamente}: inyección SQL (\textit{sqli}), \textit{Cross-Site Scripting}
(\textit{xss}), inyección de comandos (\textit{cmdi}) y \textit{path traversal}
(\textit{pathtraver}), que suman 1.478~casos.

Para dirigir el escáner sobre el Benchmark se construyó un \textit{driver}
(\texttt{tools/benchmark\_runner.py}) que \textbf{reutiliza sin modificación los
módulos de detección reales del framework} (\texttt{SQLInjectionScanner},
\texttt{XSSScanner}, \texttt{CommandInjectionScanner} y \texttt{LFIScanner}):
enumera los casos a partir de los índices que publica la propia aplicación,
descarga cada caso y determina su \textbf{vector de inyección}. El Benchmark no
siempre transporta el dato atacable en un parámetro: según el caso lo envía por
una cabecera HTTP, en el nombre de un parámetro, o en un cuerpo XML mediante
peticiones AJAX. Estos vectores ---el 34\% de los casos enumerados--- escapan al
modelo de ataque del motor (que opera sobre parámetros \texttt{GET}/\texttt{POST}
y cuerpos JSON) y se contabilizan honestamente como no detectados, sin maquillaje.
El cálculo de los hallazgos se realiza \textit{a ciegas}, sin que el detector
consulte en ningún momento la etiqueta del caso. Por rigor, el resultado se
informa en dos medidas complementarias: una \textbf{global} sobre la totalidad de
las cuatro categorías (medida conservadora, donde los vectores inalcanzables
penalizan como falsos negativos) y otra sobre el \textbf{subconjunto realmente
atacable} por el motor ---918~casos cuyo vector es un parámetro---, que aísla la
capacidad de detección del framework cuando la superficie es alcanzable.

\subsubsection{Resultados}

La Tabla~\ref{tab:benchmark-atacable} presenta la medida sobre el subconjunto
atacable, que constituye la evaluación justa de la capacidad del motor, y la
Tabla~\ref{tab:benchmark-global} la medida global conservadora.

\begin{table}[H]
    \centering
    \caption{Efectividad sobre el subconjunto atacable del OWASP Benchmark (918~casos con vector de parámetro)}
    \label{tab:benchmark-atacable}
    \small
    \begin{tabular}{l c c c c c c c}
        \toprule
        \textbf{Categoría} & \textbf{VP} & \textbf{FN} & \textbf{FP} & \textbf{VN} & \textbf{TPR} & \textbf{FPR} & \textbf{Youden} \\
        \midrule
        SQL Injection      & 156 &   0 & 17 & 133 & 1.00 & 0.11 & \textbf{0.89} \\
        Cross-Site Scripting & 137 &  20 &  0 & 157 & 0.87 & 0.00 & \textbf{0.87} \\
        Command Injection  &  31 &  37 &  0 &  80 & 0.46 & 0.00 & 0.46 \\
        Path Traversal     &   0 &  72 &  0 &  78 & 0.00 & 0.00 & 0.00 \\
        \midrule
        \textbf{TOTAL}     & \textbf{324} & \textbf{129} & \textbf{17} & \textbf{448} & \textbf{0.72} & \textbf{0.04} & \textbf{0.68} \\
        \bottomrule
    \end{tabular}

    \vspace{0.4em}
    {\footnotesize Precisión: 0.95 \quad F1: 0.82}
\end{table}

\begin{table}[H]
    \centering
    \caption{Efectividad global sobre las cuatro categorías cubiertas (1.478~casos; medida conservadora)}
    \label{tab:benchmark-global}
    \small
    \begin{tabular}{l c c c c c c c}
        \toprule
        \textbf{Categoría} & \textbf{VP} & \textbf{FN} & \textbf{FP} & \textbf{VN} & \textbf{TPR} & \textbf{FPR} & \textbf{Youden} \\
        \midrule
        SQL Injection      & 156 & 116 & 17 & 215 & 0.57 & 0.07 & 0.50 \\
        Cross-Site Scripting & 137 & 109 &  0 & 209 & 0.56 & 0.00 & 0.56 \\
        Command Injection  &  31 &  95 &  0 & 125 & 0.25 & 0.00 & 0.25 \\
        Path Traversal     &   0 & 133 &  0 & 135 & 0.00 & 0.00 & 0.00 \\
        \midrule
        \textbf{TOTAL}     & \textbf{324} & \textbf{453} & \textbf{17} & \textbf{684} & \textbf{0.42} & \textbf{0.02} & \textbf{0.39} \\
        \bottomrule
    \end{tabular}

    \vspace{0.4em}
    {\footnotesize Precisión: 0.95 \quad F1: 0.58}
\end{table}

\subsubsection{Interpretación}

Los módulos de \textbf{inyección SQL y XSS constituyen las fortalezas del
framework}. La inyección SQL alcanza un recall perfecto ($TPR = 1{,}00$) sobre el
subconjunto atacable y un Índice de Youden de \textbf{0,89}, sustentado en la
propagación de la excepción \texttt{java.sql.SQLException} que el servidor (HSQLDB
sobre Tomcat) devuelve ante los \textit{payloads} de ruptura ---la misma estrategia
basada en errores que demostró su validez en SQLite con Juice Shop. El módulo de
XSS reflejado, que inyecta el \textit{payload} canario tanto en parámetros
\texttt{GET} como en cuerpos \texttt{POST}, alcanza un Youden de \textbf{0,87}
($TPR = 0{,}87$) sin un solo falso positivo. La \textbf{precisión global de 0,95}
confirma el principio de diseño priorizado por VulnSeeker: el escáner casi nunca
inventa vulnerabilidades. El Youden de \textbf{0,68} sobre el subconjunto atacable
y de \textbf{0,39} en la medida global conservadora sitúan al framework en un rango
competitivo frente a los escáneres DAST de código abierto reportados en la
literatura (Capítulo~II), un resultado creíble que no se ha sobreestimado.

El análisis honesto de los límites es tan relevante como el de las fortalezas:

\begin{itemize}
    \item \textbf{Path Traversal ciego ($TPR = 0$):} el Benchmark no refleja en la
    respuesta el contenido del archivo solicitado, de modo que la técnica de
    VulnSeeker ---que busca firmas como \texttt{root:x:0:0} en el cuerpo--- no
    dispone de señal observable. Para atacar este límite se incorporó una técnica
    complementaria basada en errores; su resultado ---y la razón medida por la que
    el Benchmark tampoco la expone--- se analiza en el apartado siguiente.

    \item \textbf{XSS: recall residual por contexto de inyección ($TPR = 0{,}87$):}
    tras extender el módulo a cuerpos \texttt{POST}, detecta la gran mayoría de los
    casos reflejados sin un solo falso positivo; los 20~falsos negativos restantes
    corresponden a inyecciones en contextos donde el canario \texttt{<VulnSeekerXSS>}
    no se refleja de forma literal (atributos HTML o bloques JavaScript), cuya
    explotación requeriría \textit{payloads} sensibles al contexto.

    \item \textbf{Falsos positivos en SQL Injection ($FPR = 0{,}11$):} 17~casos
    etiquetados como seguros propagan igualmente una excepción de sintaxis SQL
    pese a emplear una defensa que el Benchmark considera adecuada. Un detector
    dinámico basado en errores no puede distinguirlos de los vulnerables; se trata
    de una limitación intrínseca de la técnica DAST, compartida por las
    herramientas comerciales, y no de un defecto de implementación.

    \item \textbf{Inyección de comandos parcial ($TPR = 0{,}46$):} VulnSeeker
    detecta los casos en que la salida del comando se refleja en la respuesta
    (p.~ej. \texttt{cat /etc/passwd}), pero es ciego ante la ejecución que no altera
    el cuerpo observable. Este escenario motivó la incorporación de detección ciega
    basada en tiempo, cuyo alcance ---acotado por el diseño del propio banco--- se
    analiza en el apartado siguiente.

    \item \textbf{Vectores fuera del modelo de ataque:} el 34\% de los casos inyecta
    por cabecera, nombre de parámetro o cuerpo XML, vectores no contemplados por el
    motor actual y que la medida global penaliza como falsos negativos.
\end{itemize}

\subsubsection{Extensión del motor y límites del oráculo para la detección ciega}

Los dos límites anteriores ---\textit{path traversal} y \textit{command injection}
ciegos--- motivaron una extensión deliberada del motor cuyo resultado constituye,
en sí mismo, un hallazgo metodológico. Se unificó la enumeración de vectores de los
módulos de inyección en un componente compartido
(\texttt{modules/injection\_points.py}), sobre el que se incorporaron dos técnicas
de detección ciega: (i)~\textbf{detección basada en tiempo} para \textit{command
injection}, que inyecta retardos controlados (\texttt{sleep}/\texttt{ping}) y
confirma la vulnerabilidad sólo cuando la latencia escala de forma proporcional al
duplicar el retardo ($d$ y $2d$); y (ii)~\textbf{detección basada en errores} para
\textit{path traversal}, que reconoce el rastro de excepciones de archivo del
servidor (\texttt{java.io.FileNotFoundException} y análogas). Ambas se respaldaron
con pruebas unitarias específicas que verifican su lógica de forma aislada.

La re-medición contra el Benchmark tras esta extensión arrojó un resultado
revelador: \textbf{las métricas se mantuvieron idénticas} (Youden 0,68 en el
subconjunto atacable; \textit{command injection} 0,46; \textit{path traversal}
0,00). La ausencia de regresión confirma que el refactor preservó el comportamiento
observable; la ausencia de mejora, sin embargo, no obedece a un defecto de las
nuevas técnicas, sino a una propiedad del propio oráculo, que se verificó
empíricamente sobre sus casos etiquetados:

\begin{itemize}
    \item Al inyectar un retardo de 10~segundos (\texttt{; sleep 10}) en casos de
    \textit{command injection} etiquetados como vulnerables pero sin reflejo, la
    respuesta HTTP siguió resolviéndose en $\sim$0,06~segundos: \textbf{el Benchmark
    ejecuta el comando sin bloquear la respuesta}, de modo que no existe canal
    temporal observable que la técnica basada en tiempo pueda medir.

    \item En casos de \textit{path traversal} vulnerables, el cuerpo de respuesta
    ante un \textit{payload} de recorrido de directorios resultó ser un texto breve
    ($\sim$100~bytes) \textbf{sin rastro alguno de excepción}: el Benchmark no
    propaga el error de archivo al cliente, por lo que la técnica basada en errores
    carece de señal que reconocer. Cabe destacar que esta extensión \textbf{no
    introdujo un solo falso positivo}, lo que refuta el riesgo previsto de que las
    firmas de error inflaran la $FPR$.
\end{itemize}

Este contraste es una contribución metodológica: el OWASP~Benchmark, por su
naturaleza de banco sintético, \textbf{no expone los canales laterales ---temporal
y de error--- sobre los que operan las técnicas de detección ciega}, por lo que
subestima de forma sistemática la capacidad de un escáner frente a estas variantes.
La eficacia de las técnicas incorporadas se materializa, en cambio, en aplicaciones
reales como DVWA ---donde la ejecución del comando sí bloquea la respuesta y las
aplicaciones sí propagan trazas de error---, escenario en que la validación
cualitativa de este mismo capítulo confirmó su funcionamiento. Su detección en el
Benchmark no exigiría, por tanto, una mejor heurística, sino canales fuera de banda
(\textit{out-of-band}) que el banco tampoco provee.

Esta evaluación cuantitativa cumple el cuarto objetivo específico al aportar el
Índice de Youden exacto del framework sobre un \textit{ground truth} etiquetado, y
delimita con precisión el dominio de eficacia de VulnSeeker: sobresaliente en
inyección SQL, sólido en precisión, y con líneas de mejora claramente
identificadas ---la ampliación del modelo de ataque a los vectores de cabecera y
cuerpo XML y los \textit{payloads} de XSS sensibles al contexto--- que se recogen
en las recomendaciones. La detección ciega por tiempo y por error, incorporada al
motor durante este trabajo, quedó acotada no por la heurística sino por los límites
del oráculo, según se detalló arriba.
...
por los resultados de la suite de pruebas unitarias: los 194~tests
...
\begin{enumerate}
    ...
    \item \textbf{Objetivo 4 — Validación experimental:} Las 180~pruebas
    ...
    observados en los módulos de inyección, y se complementaron con la plataforma
    pública Altoro Mutual y la SPA OWASP~Juice~Shop. La introducción de
    ...
    Finalmente, la validación cuantitativa contra el OWASP~Benchmark ---único
    entorno con \textit{ground truth} etiquetado--- arrojó un Índice de Youden de
    0,68 sobre el subconjunto atacable (0,39 en la medida global conservadora) y
    una precisión de 0,95, con la inyección SQL ($TPR = 1{,}00$; Youden 0,89) y el
    XSS reflejado (Youden 0,87) como fortalezas destacadas. Estos resultados validan la tesis central de
    Abdulghaffar et~al. (\citeyear*{abdulghaffar2023}): la integración de
    múltiples motores en una arquitectura concurrente amplía la cobertura,
    abarcando 8~categorías del OWASP Top~10.
\end{enumerate}
...
\begin{enumerate}
    ...
    HTTP y ---en SQL Injection y \textit{command injection}--- detección ciega
    basada en tiempo (\textit{time-based}) mediante la inyección de retardos
    controlados con confirmación proporcional. No obstante, otras variantes avanzadas como el
    ...
    como trabajo futuro. La cuantificación del \textit{recall} y del Índice de
    Youden, en cambio, se llevó a cabo contra el OWASP~Benchmark (Capítulo~IV) y
    delimitó con precisión el dominio de eficacia del framework: reveló como
    límites concretos la detección ciega de \textit{path traversal} y \textit{command
    injection} ---abordada mediante técnicas basadas en errores y en tiempo cuyo
    efecto, no obstante, el banco sintético no llega a exponer al no propagar ni el
    retardo del comando ni la traza de error--- y los vectores de inyección por
    cabecera o cuerpo XML no contemplados por el motor, abordables en trabajo futuro
    sin alterar la arquitectura modular. Una tercera carencia detectada en
    esa misma medición ---la cobertura de XSS limitada al método \texttt{GET}--- fue
    subsanada extendiendo el módulo a cuerpos \texttt{POST}, lo que elevó su Índice
    de Youden de 0,38 a 0,87.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item A partir de los límites cuantificados contra el OWASP Benchmark
    (Capítulo~IV), continuar ampliando el modelo de ataque del motor para elevar el
    Índice de Youden: soportar los vectores de inyección por cabecera y cuerpo XML,
    añadir \textit{payloads} de XSS sensibles al contexto (atributos HTML y bloques
    JavaScript) y complementar la detección ciega ya incorporada ---basada en tiempo
    y en errores--- con canales fuera de banda (\textit{out-of-band}), que
    permitirían evidenciar las variantes ciegas que el banco sintético no expone y
    una comparación aún más directa con los motores comerciales sobre el mismo
    \textit{ground truth}.
...
Estas capas operan de forma encadenada. La Figura~\ref{fig:flujo} sintetiza el
flujo de ejecución completo, desde la recepción del objetivo hasta la generación
de los reportes, e incorpora la etapa de descubrimiento API-aware que extiende el
alcance del crawler a las aplicaciones de página única (SPA).
...
\begin{tikzpicture}[
    paso/.style={rectangle, rounded corners, draw=black, thick,
                 text width=10.2cm, align=center, font=\small, minimum height=9mm,
                 inner sep=4pt},
    flecha/.style={-{Latex[length=2.8mm]}, thick}
]
    \node[paso, fill=gray!12] (in)
        {\textbf{URL objetivo} y configuración (GUI / CLI)};
    \node[paso, fill=blue!9, below=6mm of in] (rec)
        {\textbf{Reconocimiento:} OSINT de subdominios (crt.sh) $\rightarrow$
         \emph{fingerprinting} de tecnologías $\rightarrow$ \emph{crawler}
         autenticado (URLs, formularios y parámetros)};
    \node[paso, fill=blue!9, below=6mm of rec] (api)
        {\textbf{Descubrimiento API-aware:} especificación OpenAPI/Swagger y rutas
         \texttt{/api}, \texttt{/rest} extraídas del JavaScript, para SPAs};
    \node[paso, fill=orange!13, below=6mm of api] (atk)
        {\textbf{Ataque multihilo:} 25 módulos en paralelo\\
         (SQLi, XSS, RCE, LFI, RFI, SSRF, CORS, CSRF, CVE, TLS\dots)};
    \node[paso, fill=orange!13, below=6mm of atk] (dedup)
        {\textbf{Deduplicación} de hallazgos (nombre + URL + \emph{payload})};
    \node[paso, fill=green!13, below=6mm of dedup] (ia)
        {\textbf{Análisis con IA} (Llama 3.3 70B): informe ejecutivo y
         priorización por riesgo de negocio};
    \node[paso, fill=gray!12, below=6mm of ia] (out)
        {\textbf{Reportes:} PDF · JSON · CSV \;y\; persistencia histórica en SQLite};
    \draw[flecha] (in) -- (rec);
    \draw[flecha] (rec) -- (api);
    \draw[flecha] (api) -- (atk);
    \draw[flecha] (atk) -- (dedup);
    \draw[flecha] (dedup) -- (ia);
    \draw[flecha] (ia) -- (out);
\end{tikzpicture}
\caption{Flujo de ejecución del proceso de escaneo de VulnSeeker}
\label{fig:flujo}
\end{figure}
