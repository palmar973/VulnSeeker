...
lo que genera fatiga de alertas en los equipos de desarrollo. El diseño de
VulnSeeker prioriza la maximización del Índice de Youden en sus módulos de
inyección activa, implementando reglas de validación multi-paso y análisis de
firmas de error para mitigar la aparición de falsos positivos ($FPR \to 0$),
incluso si esto representa una reducción menor en la tasa general de cobertura.
...
\begin{enumerate}
    ...
    pruebas directas, utilizando como estándar de referencia las categorías
    del OWASP Top~10 y las métricas propuestas por el OWASP Benchmark
    Project (\cite{owaspbenchmark}): True Positive Rate (TPR), False
    Positive Rate (FPR) e Índice de Youden ($J = TPR - FPR$).
...
madurez y portabilidad. La implementación se dividió en dos bloques paralelos de
...
diagnóstica. El primer nivel consistió en la construcción de una suite de 134~pruebas
...
Con el propósito de evaluar la efectividad del framework en un entorno de producción
no controlado y desplegado sobre la internet pública, se ejecutó una auditoría dinámica
(Scan~\#93) contra el portal financiero de prueba Altoro Mutual (\texttt{demo.testfire.net}).
...
El escaneo activo identifcó un total de 268~hallazgos válidos distribuidos en diferentes
...
Este escaneo externo valida la robustez de VulnSeeker en producción: la herramienta
...
por los resultados de la suite de pruebas unitarias: los 134~tests
...
\begin{enumerate}
    \item \textbf{Costo cero de operación:} Como se detalla en la
...
\begin{enumerate}
    ...
    de la detección estática, posicionando a VulnSeeker como una de las
    primeras herramientas DAST de código abierto en integrar análisis con
    ...
    los módulos de inyección. Estos resultados validan la tesis central de
    Abdulghaffar et~al. (\citeyear*{abdulghaffar2023}): la integración de
    múltiples motores en una arquitectura concurrente maximiza la tasa de
    detección, tal como se evidenció en los 6.302~hallazgos distribuidos
    en 8~categorías OWASP.
\end{enumerate}
