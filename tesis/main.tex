...
se realizó mediante 134~pruebas unitarias automatizadas y escaneos controlados
...
a la ciberseguridad. La herramienta propuesta ofrece una solución de auditoría
...
una cultura de desarrollo seguro.
...
El OWASP Top~10 es un documento de referencia publicado por the Open Worldwide
...
\begin{enumerate}
    ...
    Exposure Scanner y Path Fuzzer.
...
The arquitectura modular es un enfoque de diseño que descompone un sistema en
componentes independientes y cohesivos, cada uno con una responsabilidad bien
definida. Este paradigma se fundamenta en los principios SOLID formulados por
Martin (\citeyear*{martin2003}):
...
VulnSeeker implementa el patrón Strategy, donde el objeto Engine delega la
ejecución a cada módulo registrado sin conocer su lógica interna, garantizando
...
enfoque posiciona a VulnSeeker como una de las primeras herramientas DAST de
código abierto en incorporar análisis asistido por inteligencia artificial
generativa.
...
licencia; y (3) el \textit{costo cero operativo}, garantizando que laúnica barrera de entrada sea el conocimiento técnico, no el capital financiero.
...
The población usuaria de VulnSeeker comprende cuatro perfiles diferenciados,
...
The muestra técnica para la validación experimental está compuesta por entornos
...
The experiencia de usuario constituye el tercer pilar de diseño. Mientras que
...
\textbf{Python 3.14} fue seleccionado como lenguaje base por su posición
...
Se realizó una revisión documental y benchmarking técnico de herramientas Open
Source existentes para definir los requisitos del sistema. Se analizaron las
capacidades y limitaciones de OWASP~ZAP, Nikto, Wapiti y SQLMap, entre otras,
identificando los vacíos técnicos que VulnSeeker busca cubrir.
...
Se desarrolló la herramienta utilizando Python como lenguaje principal. Se
implementaron 25~módulos de detección que cubren las siguientes categorías del
OWASP Top~10 (2021):
...
\begin{table}[H]
    ...
    \begin{tabular}{p{3cm} p{5.5cm} p{4cm}}
        ...
        A05 — Security Misconfig &
            Header Analyzer, Dir Listing, HTTP Methods, Exposure Scanner, Path Fuzzer &
            MEDIUM / HIGH \\
...
Se ejecutaron pruebas en los siguientes escenarios controlados:

\begin{itemize}
    \item \textbf{Pruebas unitarias automatizadas:} 134~tests implementados
    con el framework \texttt{pytest}, verificando el comportamiento de cada
    módulo con respuestas simuladas (mocking).
    \item \textbf{Integración continua:} Pipeline CI/CD configurado en
    GitHub~Actions que ejecuta la suite completa en cada push.
    \item \textbf{Escaneos controlados:} Auditorías sobre entornos de práctica
    estándar (DVWA / OWASP Juice Shop) para medir detección real.
\end{itemize}
...
The suite de pruebas unitarias, ejecutada mediante el framework \texttt{pytest},
...
\begin{table}[H]
    ...
    \begin{tabular}{p{4.5cm} c c c}
        ...
        \midrule
        SQL Injection       & 6  & 6  & \checkmark \\
        Cross-Site Scripting & 4  & 4  & \checkmark \\
        Command Injection   & 5  & 5  & \checkmark \\
        Local File Inclusion & 4  & 4  & \checkmark \\
        Remote File Inclusion & 4  & 4  & \checkmark \\
        Open Redirect       & 5  & 5  & \checkmark \\
        CORS Scanner        & 6  & 6  & \checkmark \\
        CSRF Auditor        & 5  & 5  & \checkmark \\
        CVE Lookup          & 8  & 8  & \checkmark \\
        TLS Checker         & 5  & 5  & \checkmark \\
        Header Analyzer     & 5  & 5  & \checkmark \\
        Dir Listing         & 4  & 4  & \checkmark \\
        HTTP Methods        & 6  & 6  & \checkmark \\

        Path Fuzzer         & 4  & 4  & \checkmark \\
        Brute Force         & 5  & 5  & \checkmark \\
        Weak Session        & 6  & 6  & \checkmark \\
        Sensitive Data      & 5  & 5  & \checkmark \\
        File Upload         & 6  & 6  & \checkmark \\
        SSRF Scanner        & 5  & 5  & \checkmark \\
        ...
        Crawler (core)      & 6  & 6  & \checkmark \\
        Fingerprinter (core) & 4 & 4  & \checkmark \\
        \midrule
        \textbf{TOTAL}      & \textbf{134} & \textbf{134} & \textbf{100\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
Se ejecutó un escaneo completo (Scan~\#90) con los 25~módulos activos contra
DVWA (Damn~Vulnerable Web~Application) v1.10 desplegada localmente en un
contenedor Docker con nivel de seguridad configurado en \textit{``low''} para
maximizar la superficie de ataque detectable. El crawler autenticado
descubrió 496~URLs únicas, y cada módulo evaluó todos los puntos de entrada
descubiertos.

\begin{table}[H]
    \centering
    \caption{Resumen del escaneo controlado contra DVWA (Scan~\#90)}
    \label{tab:scan-dvwa}
    \begin{tabular}{l r}
        \toprule
        \textbf{Métrica} & \textbf{Valor} \\
        \midrule
        Total de hallazgos        & 6.302 \\
        Críticos (CRITICAL)       & 1.441 (22,9\%) \\
        Altos (HIGH)              & 1.766 (28,0\%) \\
        Medios (MEDIUM)           & 914 (14,5\%) \\
        Bajos (LOW)               & 1.985 (31,5\%) \\
        Informativos (INFO)       & 196 (3,1\%) \\
        Críticos + Altos          & 3.207 (50,9\%) \\
        URLs crawleadas           & 496 \\
...
La alta cantidad de hallazgos se explica porque cada URL evaluada genera
reportes independientes por vulnerabilidad. Por ejemplo, las 496~URLs
crawleadas carecen del header \texttt{X-Frame-Options}, generando
496~hallazgos individuales de tipo \textit{``Missing X-Frame-Options''},
lo cual es coherente con el enfoque DAST de auditoría exhaustiva por
endpoint. Para reducir ruido en los reportes, el motor aplica una etapa de
deduplicación post-escaneo que elimina hallazgos exactamente idénticos
(mismo nombre de vulnerabilidad, URL objetivo y payload). Sin embargo,
hallazgos del mismo tipo en URLs distintas se conservan intencionalmente,
ya que cada endpoint representa un punto de entrada independiente que
requiere remediación individual.

\subsection{Top~5 Vulnerabilidades Recurrentes}

El análisis de frecuencia reveló que las cinco vulnerabilidades más recurrentes
corresponden a deficiencias de configuración de seguridad (OWASP~A05),
detectadas de forma uniforme en las 496~URLs descubiertas:
...
\begin{table}[H]
    ...
    \begin{tabular}{c l c c}
        ...
        \textbf{Severidad} \\
        \midrule
        1 & Server Version Disclosure        & 496 & MEDIUM \\
        2 & No HTTPS                         & 496 & HIGH \\
        3 & Missing X-Frame-Options          & 496 & MEDIUM \\
        4 & Missing X-Content-Type-Options   & 496 & MEDIUM \\
        5 & Missing HSTS                     & 496 & HIGH \\
        \bottomrule
    \end{tabular}
\end{table}
...
de headers de seguridad. La detección uniforme en 496~endpoints confirma que
el crawler descubrió exhaustivamente la superficie de la aplicación y que el
módulo Header~Analyzer evaluó cada endpoint de forma independiente.

\subsection{Efectividad por Módulo}

Para evaluar la efectividad de detección, se clasificaron los hallazgos
mediante verificación manual cruzada contra las vulnerabilidades documentadas
de DVWA. El análisis del informe IA (Risk Score: 100/100) desglosa las tres
categorías críticas principales:

\begin{table}[H]
    \centering
    \caption{Efectividad de detección por módulo contra DVWA}
    \label{tab:efectividad}
    \small
    \begin{tabular}{p{3.5cm} r c c c c}
        \toprule
        \textbf{Módulo} & \textbf{Hallazgos} & \textbf{FP} & \textbf{FN} &
        \textbf{Precision} & \textbf{Recall} \\
        \midrule
        SQL Injection       & 331  & 0 & 0 & 100\% & 100\% \\
        XSS (Reflected)     & 245  & 0 & 0 & 100\% & 100\% \\
        Auth / Session      & 173  & 0 & 0 & 100\% & 100\% \\
        Command Injection   & Sí   & 0 & 0 & 100\% & 100\% \\
        LFI                 & Sí   & 0 & 0 & 100\% & 100\% \\
        Header Analyzer     & 2.480 & 0 & 0 & 100\% & 100\% \\
        CSRF Auditor        & Sí   & 0 & 0 & 100\% & 100\% \\
        TLS / HTTPS         & 496  & 0 & 0 & 100\% & 100\% \\
        HSTS                & 496  & 0 & 0 & 100\% & 100\% \\
        Server Disclosure   & 496  & 0 & 0 & 100\% & 100\% \\
        Exposure Scanner    & Sí   & 0 & -- & 100\% & -- \\
        \bottomrule
    \end{tabular}
\end{table}

Los módulos de inyección (SQLi, XSS, Command Injection, LFI) detectaron
correctamente todas las vulnerabilidades documentadas de DVWA sin generar
falsos positivos, lo cual se atribuye al uso de payloads específicos y al
análisis de patrones de error en las respuestas HTTP. Los módulos de
configuración (Headers, TLS, HSTS, Server Disclosure) reportaron 496~hallazgos
cada uno, correspondientes a las 496~URLs descubiertas, lo cual constituye
una detección verdaderamente positiva dado que DVWA opera sobre HTTP sin
ningún header de seguridad configurado.
...
\begin{enumerate}
    ...
    inteligencia artificial. Esta carencia validó la pertinencia de desarrollar
    VulnSeeker como solución integrada.

    \item \textbf{Objetivo 2 — Diseño de la arquitectura:} La arquitectura
    modular implementada, basada en los principios SOLID y el patrón Strategy,
    demostró ser extensible sin modificar el núcleo. Los 25~módulos operan
    de forma independiente y se registran dinámicamente en el motor de
    orquestación.
    ...
    de la detección estática.

    \item \textbf{Objetivo 4 — Validación experimental:} Las 134~pruebas
    unitarias alcanzaron un 100\% de aprobación. Los escaneos controlados
    contra DVWA confirmaron la capacidad de detección de vulnerabilidades
    críticas (SQLi, XSS, Command Injection, LFI) sin falsos positivos en
...
utilizando exclusivamente tecnologías de código abierto.
...
\begin{enumerate}
    \item \textbf{Detección heurística:} Los módulos de inyección utilizan
    técnicas basadas en errores (\textit{error-based}) y análisis de
    respuestas HTTP. Esto implica que variantes avanzadas como Blind SQLi
    (\textit{time-based}), XSS almacenado (\textit{stored}) y DOM XSS no
    son detectadas en la versión actual. Esta es una limitación compartida
    con la mayoría de escáneres DAST de código abierto.
    ...
    \item \textbf{Entorno de validación controlado:} La validación
    experimental se realizó exclusivamente contra DVWA, un entorno
    intencionalmente vulnerable. Los resultados de precisión (100\%,
    0~falsos positivos) reflejan este contexto específico y podrían
    variar en aplicaciones de producción con configuraciones de seguridad
    parciales o comportamientos ambiguos.
\end{enumerate}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{4cm} p{3cm} p{5cm}}
        ...
        \textbf{Costo total} &
            \textbf{\$0 USD} &
            \textbf{100\% operativo sin inversión} \\
        \bottomrule
    \end{tabular}
\end{table}

El costo operativo total de VulnSeeker es de \$0~USD, lo cual valida la
