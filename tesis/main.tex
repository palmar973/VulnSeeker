...
se realizó mediante 178~pruebas unitarias automatizadas, escaneos controlados
...
diagnóstica. El primer nivel consistió en la construcción de una suite de 178~pruebas
...
\begin{table}[H]
    ...
    \begin{tabular}{p{4.5cm} c c c}
        ...
        \midrule
        \textbf{TOTAL}      & \textbf{178} & \textbf{178} & \textbf{100\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c c c c c}
        ...
        Cross-Site Scripting &  60 &  97 &  0 & 157 & 0.38 & 0.00 & 0.38 \\
        Path Traversal     &   0 &  72 &  0 &  78 & 0.00 & 0.00 & 0.00 \\
        \midrule
        \textbf{TOTAL}     & \textbf{247} & \textbf{206} & \textbf{17} & \textbf{448} & \textbf{0.55} & \textbf{0.04} & \textbf{0.51} \\
        \bottomrule
    \end{tabular}

    \vspace{0.4em}
    {\footnotesize Precisión: 0.94 \quad F1: 0.69}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c c c c c}
        ...
        Cross-Site Scripting &  60 & 186 &  0 & 209 & 0.24 & 0.00 & 0.24 \\
        Path Traversal     &   0 & 133 &  0 & 135 & 0.00 & 0.00 & 0.00 \\
        \midrule
        \textbf{TOTAL}     & \textbf{247} & \textbf{530} & \textbf{17} & \textbf{684} & \textbf{0.32} & \textbf{0.02} & \textbf{0.29} \\
        \bottomrule
    \end{tabular}

    \vspace{0.4em}
    {\footnotesize Precisión: 0.94 \quad F1: 0.47}
\end{table}

\subsubsection{Interpretación}

El módulo de \textbf{inyección SQL constituye la fortaleza del framework}: alcanza
un recall perfecto ($TPR = 1{,}00$) sobre el subconjunto atacable y un Índice de
Youden de \textbf{0,89}, sustentado en la propagación de la excepción
\texttt{java.sql.SQLException} que el servidor (HSQLDB sobre Tomcat) devuelve ante
los \textit{payloads} de ruptura ---la misma estrategia basada en errores que
demostró su validez en SQLite con Juice Shop. La \textbf{precisión global de 0,94}
confirma el principio de diseño priorizado por VulnSeeker: el escáner casi nunca
inventa vulnerabilidades. El Youden de 0,51 sobre el subconjunto atacable y de
0,29 en la medida global conservadora sitúan al framework en un rango comparable
al de los escáneres DAST de código abierto reportados en la literatura
(Capítulo~II), un resultado creíble que no se ha sobreestimado.
...
\begin{itemize}
    ...
    \item \textbf{XSS limitado al método GET ($TPR = 0{,}38$):} el módulo verifica
    el reflejo del \textit{payload}, que se produce en la totalidad de los casos,
    pero sólo inyecta en parámetros \texttt{GET}; los casos servidos por
    \texttt{POST} se pierden. Extender el módulo a cuerpos \texttt{POST} ---como ya
    se hizo con SQL Injection--- elevaría directamente su recall.
...
por los resultados de la suite de pruebas unitarias: los 178~tests
...
\begin{enumerate}
    ...
    \item \textbf{Objetivo 4 — Validación experimental:} Las 178~pruebas
    ...
    0,51 sobre el subconjunto atacable (0,29 en la medida global conservadora) y
    una precisión de 0,94, con la inyección SQL como fortaleza destacada
    ($TPR = 1{,}00$; Youden 0,89). Estos resultados validan la tesis central de
...
\begin{enumerate}
    ...
    respuesta no refleja el contenido del archivo), la cobertura de XSS
    restringida al método \texttt{GET} y los vectores de inyección por cabecera o
    cuerpo XML no contemplados por el motor; todos ellos abordables en trabajo
    futuro sin alterar la arquitectura modular.
\end{enumerate}
...
\begin{enumerate}
    ...
    (Capítulo~IV), ampliar el modelo de ataque del motor para elevar el Índice de
    Youden: extender el módulo XSS a cuerpos \texttt{POST}, incorporar detección de
    inyección ciega basada en tiempo (\textit{path traversal} y \textit{command
    injection} sin reflejo en la respuesta) y soportar los vectores de cabecera y
    cuerpo XML, lo que permitiría una comparación aún más directa con los motores
